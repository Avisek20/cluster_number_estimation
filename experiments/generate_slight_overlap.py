...
def generate_slight_overlap(num_clusters, cluster_size, data_dim) :
    ...
    for iter1 in range(num_clusters-1) :
        ...
        while flag :
            if rand <= overlap_probability:
                #if infinite_counter % 10000 == 0:
                #    print(infinite_counter // 10000)
                # Pick a center
                which_center = np.random.randint(iter1+1)
                # Add noise to get a new center
                noise = np.random.uniform(low=-5,high=5,size=(1,data_dim))
                # New center
                mean_vector[iter1+1,:] = \
...
if __name__ == '__main__':
    ...
    directory = 'data_slight_overlap2'
    key_directory = 'key_slight_overlap2'
