...
def generate_diff_spread(num_clusters, cluster_size, data_dim):
    ...
    # Possible cluster spreads
    list_spread = [1, 2, 3, 4]
    # Generate first cluster center
    mean_vector[0,:] = \
        np.random.uniform(low=0,high=40*num_clusters,size=(1,data_dim))
    # Generate first cluster
    random_spread = list_spread[np.random.randint(len(list_spread))]
    data_set = np.hstack((\
        normal(loc=mean_vector[0,:], scale=random_spread, \
        size=(cluster_size, data_dim)), np.zeros((cluster_size, 1)) \
        ))
    for iter1 in range(num_clusters-1) :
        # Generate next centers at distance > 40 from all other centers
        flag = 1
        while flag :
            ...
            # Find minimum distance, if less than 40, loop again to
        ...
        data_set = np.vstack((data_set, \
            np.hstack((\
                normal(loc=mean_vector[iter1+1,:], scale=random_spread, \
